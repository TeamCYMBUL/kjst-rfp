...
Deno.serve(async (req: Request) => {
  ...
  const { data: inv, error: invErr } = await supabase
    ...
    .select('id, status, revoked_at, expires_at, submitted_at, reopened_at, original_bid, hotel_name, hotel_contact_name, hotel_contact_email, token, trips(id, client_id, opponent_label, city, arrival_date, departure_date, clients(team_name))')
    .eq('token', token)
    .single()
  ...
  if ((inv as any).expires_at && new Date((inv as any).expires_at).getTime() < Date.now())
    return json({ error: 'This link has expired. Please contact KJST for a new one.' }, 403)
