...
async function sendResend(payload: unknown, apiKey: string, maxAttempts = 4): Promise<Response> {
  for (let attempt = 1; ; attempt++) {
    ...
    if (res.status !== 429 || attempt >= maxAttempts) {
      if (!res.ok) await logServerError('send-invitation:resend', new Error(`Resend ${res.status}: ${(await res.clone().text()).slice(0, 200)}`))
      return res
    }
...
Deno.serve(async (req: Request) => {
  ...
  const { invitation_id, base_url = Deno.env.get('SITE_URL') ?? 'https://rfp.kjsportstravel.com' } = body
