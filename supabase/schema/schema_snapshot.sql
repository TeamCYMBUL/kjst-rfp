...
TABLE contract_check_rules (id uuid, rule_text text, active boolean, kind text default 'check' check (kind in ('check','guidance')), sort_order integer, created_by text, created_at timestamp with time zone, updated_at timestamp with time zone)
TABLE contracts (id uuid, invitation_id uuid, trip_id uuid, client_id uuid, token text, status text, file_path text, file_name text, uploaded_at timestamp with time zone, signed_file_path text, signed_file_name text, signed_at timestamp with time zone, analysis jsonb, analyzed_at timestamp with time zone, analysis_status text check (analysis_status in ('running','done','error')), analysis_error text, staff_notes text, created_at timestamp with time zone, updated_at timestamp with time zone)
