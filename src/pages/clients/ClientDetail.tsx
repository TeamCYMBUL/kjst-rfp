...
export default function ClientDetail() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(async ({ data: tripRows }) => {
        if (!tripRows || tripRows.length === 0) { setHistory([]); setProposalCounts({ total: 0, unprinted: 0, awarded: 0 }); return }
        const tripIds = tripRows.map((t: any) => t.id)

        // Count submitted/awarded bids, how many are still unprinted, and how many
        // are the awarded winners (for the "print selected" checklist).
  ...
  const handleExportAllCities = async () => {
    ...
    try {
      await exportAllCitiesForClient(id!, client.team_name)
    } finally {
                      ...
                      <td className="py-2">
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {trip.status === 'closed' && (() => {
                            ...
                            return winner ? (
                              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 px-2.5 py-1 text-[11px] font-semibold leading-5 text-emerald-700 dark:text-emerald-300">
          ...
          <Card className="p-6">
            ...
            {proposalCounts === null ? (
              ...
              <div className="space-y-2">
                <a
                  href={`/clients/${id}/proposals?mode=awarded`}
                  target="_blank"
                ...
                >
                  <span>Print selected (awarded) RFPs</span>
