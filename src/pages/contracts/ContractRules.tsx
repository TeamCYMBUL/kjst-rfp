...
import type { ContractCheckRule, ContractRuleKind } from '../../lib/contractsApi'
...
export default function ContractRules() {
  ...
  const [draftKind, setDraftKind] = useState<ContractRuleKind>('check')
  ...
  const add = async () => {
    ...
    if (!text) return
    setBusy(true)
    try { await addContractRule(text, user?.email ?? null, draftKind); setDraft(''); setDraftKind('check'); load() }
  ...
  const toggleKind = async (r: ContractCheckRule) => {
    setBusy(true)
    const next: ContractRuleKind = r.kind === 'guidance' ? 'check' : 'guidance'
    try { await updateContractRule(r.id, { kind: next }); load() }
    catch (e: any) { setError(e.message ?? 'Failed to update') }
    finally { setBusy(false) }
  }
  ...
  const activeCheckCount = rules.filter((r) => r.active && r.kind !== 'guidance').length
  const activeGuidanceCount = rules.filter((r) => r.active && r.kind === 'guidance').length
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-slate-500 dark:text-slate-400">
          Plain-English rules the AI applies to every contract on top of the bid comparison.
          A <span className="font-medium text-slate-700 dark:text-slate-200">Check</span> becomes its own
          pass/fail line in the fact-check. <span className="font-medium text-slate-700 dark:text-slate-200">Guidance</span> shapes
          how the AI judges and how thoroughly it covers the contract, without adding its own line — use it for
          principles and broad coverage notes. {activeCheckCount} check{activeCheckCount === 1 ? '' : 's'}, {activeGuidanceCount} guidance active.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          <div className="inline-flex rounded-lg border border-slate-200 dark:border-slate-600 p-0.5 text-xs">
            <button
              type="button"
              onClick={() => setDraftKind('check')}
              className={`rounded-md px-2.5 py-1 font-medium ${draftKind === 'check' ? 'bg-slate-800 text-white' : 'text-slate-500 dark:text-slate-400'}`}
            >
              Check
            </button>
            <button
              type="button"
              onClick={() => setDraftKind('guidance')}
              className={`rounded-md px-2.5 py-1 font-medium ${draftKind === 'guidance' ? 'bg-indigo-600 text-white' : 'text-slate-500 dark:text-slate-400'}`}
            >
              Guidance
            </button>
          </div>
          ...
          <div key={r.id} className="flex items-start gap-3 px-5 py-4">
            <div className="mt-0.5 flex shrink-0 flex-col items-start gap-1">
              ...
              <button
                ...
                disabled={busy}
                title={r.kind === 'guidance'
                  ? 'Guidance — shapes judgment, no check line. Click to make it a Check.'
                  : 'Check — its own pass/fail line. Click to make it Guidance.'}
                className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${
                  r.kind === 'guidance'
              ...
              >
                {r.kind === 'guidance' ? 'Guidance' : 'Check'}
              </button>
            </div>
